...
class SingleResponse(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "singleExtensions",
            Extensions().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1)),
        ),
    )
...
class ResponderID(univ.Choice):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType(
            "byName",
            Name().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1)),
        ),
        namedtype.NamedType(
            "byKey",
...
class ResponseData(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.DefaultedNamedType(
            "version",
            Version("v1").subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)),
        ),
...
class TBSRequest(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "requestorName",
            GeneralName().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1)),
        ),
        namedtype.NamedType("requestList", univ.SequenceOf(componentType=Request())),
        namedtype.OptionalNamedType(
            "requestExtensions",
            Extensions().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 2)),
        ),
    )
...
class OCSPRequest(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "optionalSignature",
            Signature().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)),
        ),
    )
