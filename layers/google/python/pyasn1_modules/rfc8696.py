...
class CMSORIforPSKOtherInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "keyMgmtAlgType",
            univ.Enumerated(namedValues=namedval.NamedValues(("keyTrans", 5), ("keyAgree", 10))),
        ),
        namedtype.NamedType("keyEncryptionAlgorithm", rfc5652.KeyEncryptionAlgorithmIdentifier()),
        namedtype.NamedType(
            "pskLength", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(1, MAX))
        ),
        namedtype.NamedType(
            "kdkLength", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(1, MAX))
        ),
    )
