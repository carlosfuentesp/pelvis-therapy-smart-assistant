...
JWTClaimConstraints.componentType = namedtype.NamedTypes(
    namedtype.OptionalNamedType(
        "mustInclude",
        JWTClaimNames().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)),
    ),
    namedtype.OptionalNamedType(
        "permittedValues",
...
TelephoneNumberRange.componentType = namedtype.NamedTypes(
    ...
    namedtype.NamedType(
        "count", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(2, MAX))
    ),
)
