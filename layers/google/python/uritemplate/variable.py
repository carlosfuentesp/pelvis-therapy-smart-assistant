...
class URIVariable:
    ...
    def _query_expansion(
        ...
        if dict_test(value) or tuples:
            ...
            if explode:
                return self.join_str.join(f"{quote(k, safe)}={quote(v, safe)}" for k, v in items)
            else:
    ...
    def _label_path_expansion(
        ...
        if dict_test(value) or tuples:
            ...
            expanded = join_str.join(
                format_str % (quote(k, safe), quote(v, safe)) for k, v in items if v is not None
            )
        ...
        if dict_test(value) or tuples:
            ...
            if explode:
                return join_str.join(
                    f"{quote(k, safe)}={quote(v, safe)}" for k, v in items if v is not None
                )
            else:
                expanded = ",".join(
                    f"{quote(k, safe)},{quote(v, safe)}" for k, v in items if v is not None
    ...
    def _string_expansion(
        ...
        if dict_test(value) or tuples:
            ...
            return ",".join(
                format_str % (quote(k, self.safe), quote(v, self.safe)) for k, v in items
            )
