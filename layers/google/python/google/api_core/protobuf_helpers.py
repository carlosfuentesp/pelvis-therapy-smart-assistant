...
def check_oneof(**kwargs):
    ...
    if len(not_nones) > 1:
        raise ValueError(
            "Only one of {fields} should be set.".format(fields=", ".join(sorted(kwargs.keys())))
        )
...
def get(msg_or_dict, key, default=_SENTINEL):
    ...
    if isinstance(msg_or_dict, message.Message):
        ...
        raise TypeError(
            "get() expected a dict or protobuf message, got {!r}.".format(type(msg_or_dict))
        )
...
def set(msg_or_dict, key, value):
    ...
    if not isinstance(msg_or_dict, (collections.abc.MutableMapping, message.Message)):
        raise TypeError(
            "set() expected a dict or protobuf message, got {!r}.".format(type(msg_or_dict))
        )
