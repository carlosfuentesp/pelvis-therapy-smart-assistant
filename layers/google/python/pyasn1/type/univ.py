...
class BitString(base.SimpleAsn1Type):
    ...
    def fromOctetString(cls, value, internalFormat=False, prepend=None, padding=0):
        ...
        """
        value = SizedInteger(int.from_bytes(bytes(value), "big") >> padding).setBitLength(
            len(value) * 8 - padding
        )
    ...
    def setComponentByPosition(
        ...
        if isinstance(idx, slice):
            ...
            for subIdx, subValue in enumerate(value):
                self.setComponentByPosition(
                    startIdx + subIdx, subValue, verifyConstraints, matchTags, matchConstraints
                )
            return self
        ...
        if value is noValue:
            ...
            if not subtypeChecker(
                value, verifyConstraints and matchTags, verifyConstraints and matchConstraints
            ):
