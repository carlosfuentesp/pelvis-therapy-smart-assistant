...
class BaseMarshal:
    ...
    def register(self, proto_type: type, rule: Rule = None):
        ...
        if rule:
            ...
            if not isinstance(rule, Rule):
                raise TypeError(
                    "Marshal rule instances must implement " "`to_proto` and `to_python` methods."
                )
        ...
        def register_rule_class(rule_class: type):
            ...
            if not issubclass(rule_class, Rule):
                raise TypeError(
                    "Marshal rule subclasses must implement " "`to_proto` and `to_python` methods."
                )
    ...
    def to_proto(self, proto_type, value, *, strict: bool = False):
        ...
        if isinstance(value, dict) and (
            proto_type.DESCRIPTOR.has_options and proto_type.DESCRIPTOR.GetOptions().map_entry
        ):
